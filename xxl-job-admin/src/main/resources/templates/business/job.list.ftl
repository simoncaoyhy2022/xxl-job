                            ...
                            <select class="form-control" id="jobGroup">
                                <#list JobGroupList as group>
                                    <option value="${group.id}"
                                            <#if jobGroup==group.id>selected</#if> >${group.name}</option>
                                </#list>
                            </select>
                        </div>
                    </div>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form class="form-horizontal form" role="form">
                            ...
                            <div class="form-group">
                                <label for="firstname" class="col-sm-2 control-label">${I18n.jobinfo_field_category}
                                    <font color="red">*</font></label>
                                <div class="col-sm-4">
                                    <select class="form-control" name="categoryId">
                                        <option value="0">${I18n.system_please_choose}</option>
                                        <#list CategoryList as cate>
                                            <option value="${cate.id}">${cate.name}</option>
                                        </#list>
                                    </select>
    ...
    $(function () {
        ...
        $.adminTable.initAdd({
            ...
            writeFormData: function () {
                ...
                // 》默认填充 author / alarmEmail（每次打开新增框都重新赋值，避免被 form.reset() 清空）
                var _curName = '${currentUserName!}';
                var _curEmail = '${currentUserEmail!}';
                if (_curName) {
                    $("#addModal .form input[name='author']").val(_curName);
                }
                if (_curEmail) {
                    $("#addModal .form input[name='alarmEmail']").val(_curEmail);
                }
        ...
        $.adminTable.initUpdate({
            ...
            writeFormData: function (row) {
                ...
                $('#updateModal .form select[name=categoryId] option[value=' + row.categoryId + ']').prop('selected', true);
        ...
        // set default author/alarmEmail from current login user for add/update forms and search field
        try {
            var _curName = '${currentUserName!}';
            var _curEmail = '${currentUserEmail!}';
            if (_curName) {
            ...
            if (_curEmail) {
                // $("#addModal .form input[name='alarmEmail']").val(_curEmail);
                $("#updateModal .form input[name='alarmEmail']").val(_curEmail);
            }
        } catch (e) {
        ...
        $("#data_operation").on('click', '.job_copy', function () {
            ...
            $('#addModal .form select[name=categoryId] option[value=' + row.categoryId + ']').prop('selected', true);
        });
